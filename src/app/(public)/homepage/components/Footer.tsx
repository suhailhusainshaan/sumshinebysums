...
const Footer = ({ categories = [] }: FooterProps) => {
  ...
  return (
    <footer className="bg-velvet border-t-0">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12 mb-12">
            ...
            <div className="flex items-center justify-center space-x-3 mt-4">
              {socialLinks?.map((social) => (
                <a
                  ...
                  className="w-14 h-14 flex items-center justify-center bg-porcelain/10 rounded-full text-porcelain hover:bg-gold hover:text-velvet transition-luxe"
                >
          ...
          {/* Shop Links */}
          <div>
            <h3 className="font-display text-lg tracking-wide text-porcelain mb-4">Shop</h3>
            <ul className="space-y-3">
              {finalShopLinks?.map((link) => (
                <li key={link?.label}>
                  <Link
                    href={link?.path}
                    className="text-porcelain/80 hover:text-gold transition-luxe"
                  >
                    {link?.label}
                  </Link>
                </li>
          ...
          {/* Help Links */}
          <div>
            <h3 className="font-display text-lg tracking-wide text-porcelain mb-4">Help</h3>
            <ul className="space-y-3">
        ...
        <div className="pt-8 border-t border-porcelain/10">
          <div className="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
            <p className="text-xs text-porcelain/60 uppercase tracking-widest text-center sm:text-left">
              © {currentYear} Sumshine By Sums. All rights reserved.
            </p>
            <div className="flex items-center space-x-4">
              <Icon name="CreditCardIcon" size={32} className="text-porcelain/60" />
              <Icon name="ShieldCheckIcon" size={32} className="text-porcelain/60" />
            </div>
          </div>
        </div>
      </div>
